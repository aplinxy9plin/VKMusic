...
class MyMusicController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if Connectivity.isConnectedToInternet() {
            ...
            if(Global.internetConnection){
                let alert = UIAlertController(title: "Ошибка", message: "Отсутствует подключение к интернету. Вы можете прослушивать сохраненную музыку", preferredStyle: UIAlertControllerStyle.alert)
                alert.addAction(UIAlertAction(title: "Слушать", style: UIAlertActionStyle.default, handler: nil))
